...
PRO hs_calibproc, listpath=listpath, calpath=calpath,$
                  mjd=mjd, rerun=rerun, wavetweak=wavetweak, path=path, $
                  doall=doall, dobias=dobias, dodome=dodome, dosky=dosky,$
                  doarc=doarc, dowave=dowave, doapflat=doapflat, $
  ...
  if keyword_set(doall) then begin
     hs_calibproc, listpath=listpath, calpath=calpath, mjd=mjd,$
        rerun=rerun, wavetweak=wavetweak, chelle=chelle, path=path, $
        /dobias, /dodome, /dosky, /doarc, /dowave, dodark=dodark, doapflat=doapflat, qaplot=qaplot
       return
    endif
...
;;Create the biasfiles
    ...
    if keyword_set(dobias) and (file_test(listpath + 'bias.list') ne 0) then begin
    ...
    if keyword_set(dodark) then begin
       if file_test(calpath + 'dark.fits') EQ 0L then begin
          readcol, listpath + 'dark.list', file, $
             format='A', comment='#'
          hs_darkgen, file, outdir=calpath, mjd=mjd, /model
       endif
    endif
    ...
    if keyword_set(doapflat) then begin
       ;;Create the apflatfiles
       apflatfile = findfile(listpath+'apflat*')
       junk = strsplit(listpath, ' ', length = pathcount)
       if file_test(calpath + 'apflat.fits') EQ 0L then begin
          readcol, listpath + 'apflat.list', $
             file, junk, format='A,A', comment='#'
          hs_dflatgen, file,  outdir=calpath, mjd=mjd, /apflat
       endif
    endif
    ...
    if keyword_set(dosky) AND $
       ...
       if file_test(calpath + 'sflat.fits') EQ 0L then begin
          readcol,  listpath + 'sflat.list', $
             file, junk, format='A,A', comment='#'
          hs_sflatgen, file,  outdir=calpath, mjd=mjd, chelle=chelle
       endif
       if file_test('calibration/'+rerun+'/sflat_traceset.fits') eq 0L then begin
         hs_maketraceset, /write, rerun=rerun, mjd=root, chelle=chelle, flatfile='calibration/'+rerun+'/sflat.fits', outfile='calibration/'+rerun+'/sflat_traceset.fits'
       endif
    endif
    ...
    if keyword_set(dowave) then begin
       ...
       if file_test('calibration/'+rerun+'/traceset.fits') eq 0L then begin
          ;;Make the traceset
          if keyword_set(doapflat) then $
             hs_maketraceset, /write, rerun=rerun, mjd=root, chelle=chelle, flatfile='calibration/'+rerun+'/apflat.fits' else $
       ...
       if file_test('calibration/'+rerun+'/'+'wset.fits') EQ 0L then begin
          if keyword_set(wavetweak) then hs_tweakwave, arcguess, $
             rerun=rerun, root='' 
          hs_findwave, rerun=rerun, root=mjd, arcguess=arcguess, doplot=qaplot, chelle=chelle
