...
n_fib=a[2]
x=findgen(nx)
;only do this for the red end, don't mistake low S/N at blue
;end for fringes
cutoff=2180 ;pixels
for i=0,299 do begin
   lowess,x[cutoff:*],flat_orig[cutoff:*,i],4,ff1
   fringes[cutoff:*,i]=ff1
   smoothed=djs_median(fringes[*,i],width=8, boundary='reflect')
   fringes[cutoff-5:cutoff+5,i]=smoothed[cutoff-5:cutoff+5]
   fringes[*,i]=fringes[*,i]-1.0
   flat[*,i]=flat[*,i]-fringes[*,i]
endfor
stop
