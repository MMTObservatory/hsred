...
PRO hs_extract, filelist, plugfile=plugfile, skysubtract=skysubtract, $
                ...
                tellcorr=tellcorr, uberextract=uberextract, $
                ivarout=ivarout, rerun=rerun, outname=outname, $
                plateid=plateid, sky2d=sky2d, chelle=chelle, $
                qaplot=qaplot, quicklook=quicklook, quickplot=quickplot, $
                stand=stand,  checkaverage=checkaverage, $
                docosmic=docosmic, doplot=doplot, customskies=customskies, $
                superfile=superfile, useaverage=useaverage, $
  ...
  if keyword_set(quicklook) then begin   
     hs_extract, filelist, plugfile=plugfile, /tweaksky, $
        ...
        rerun=rerun, stand=stand, checkaverage=checkaverage, chelle=chelle, $
  ...
  if file_test('calibration/'+rerun+'/traceset.fits') eq 0L then begin
     ;;Make the traceset
     if keyword_set(chelle) and file_test('calibration/'+rerun+'/apflat.fits') then begin
        hs_maketraceset, /write, flatfile='calibration/'+rerun+'/apflat.fits', rerun=rerun, mjd=root,chelle=chelle 
     endif else begin
        hs_maketraceset, /write, rerun=rerun, mjd=root,chelle=chelle
     endelse
  endif
  ...
  for j = 0, n_elements(filelist) -1 do begin
     for ccdnum = 1, 2 do begin
        hs_proc, filelist(j), ccdnum, spec, specivar, pixelmask=pixmask, $
           rerun=rerun, root=root, header=objhdr, nobias=nobias, chelle=chelle,$
...
;        sigma= 2.0
        ...
        highrej = 30
        lowrej = 30
        if keyword_set(chelle) then npoly=1 else npoly = 4
        wfixed = [1, 1, 1 ]
...
ymodel=1
        ...
        if file_test('calibration/'+rerun+'/'+'wset.fits') EQ 0L then begin
           hs_findwave, rerun=rerun, root=root, chelle=chelle
        endif

        if file_test('calibration/'+rerun+'/'+'dispset.fits') EQ 0L then begin
           hs_findarcdisper, rerun=rerun, /write, chelle=chelle
        endif
        ...
        if keyword_set(plugcat) then begin
           ...
           if ccdnum eq 1 then other = 2
           if ccdnum eq 2 then other = 1
           hs_maptoplug, plugfile + '_map', plugmap, ccdnum=ccdnum, dummy=dummy, chelle=chelle
           hs_maptoplug, plugfile + '_map', plugcheck ,ccdnum=other, dummy=dummy, chelle=chelle
        ...
        divideflat, specflux, scale, invvar=specfluxivar ;changed SM 09/17 from specivar -bug?
        ...
        if keyword_set(fiberflat) then begin
           ...
           ;;correction plus fringing)
           if file_test('calibration/'+rerun+'/'+'traceflat.fits')$
              EQ 0L then begin 
              hs_makesuperflat, rerun=rerun, root=root, chelle=chelle
           endif
           ...
           if not keyword_set(nosky) then begin
              ...
              if file_test('calibration/'+rerun+'/'+'fiberflat.fits')$
                 EQ 0L then begin 
                 hs_makefiberflat, rerun=rerun, root=root, /usesky, chelle=chelle
              endif
              
              fflat = mrdfits('calibration/'+rerun+'/'+$
                              'fiberflat.fits',ccdnum-1, /silent)
              if (size(fflat))[0] eq 2 then divideflat, specflux, fflat, invvar=specfluxivar ;fixed from specivar
              sxaddpar, objhdr, 'FIBERFLAT', 'Fiber flat applied'
            endif else begin
              ;;make the fiberflats from domeflat instead of skyflat
              if file_test('calibration/'+rerun+'/'+'fiberflat.fits')$
                 EQ 0L then begin 
                 hs_makefiberflat, rerun=rerun, root=root, usesky=0, chelle=chelle
              endif
...
;;        
        if keyword_set(tweaksky) then begin
           ...
           hs_tweaksky, specflux, specfluxivar, wset, ccdnum=ccdnum, $
              ...
              arcshift=arcshift, chelle=chelle
...
;;
        ...
        if k[0] ne -1 then begin
           ...
           ;;If this is RV31 chelle, check at 5170 and 5270A
           if keyword_set(chelle) then begin
             kcheck = indgen(n_elements(k))
           
             for ifib = 0,  n_elements(k)-1 do begin
                kcheck(ifib) = (where(abs(lam11[*,ifib]-5170.0) $
                                   eq min(abs(lam11[*,ifib]-5170.0))))[0]
             endfor     
             djs_iterstat, specflux[kcheck,k], mean=mean, sigma=sigma
             w = where(specflux[kcheck,k] gt mean + 3*sigma)
             ...
             for ifib = 0,  n_elements(k)-1 do begin
                kcheck(ifib) = (where(abs(lam11[*,ifib]-5270.0) $
                                   eq min(abs(lam11[*,ifib]-5270.0))))[0]
             endfor     
             djs_iterstat, specflux[kcheck,k], mean=mean, sigma=sigma
             w = where(specflux[kcheck,k] gt mean + 3*sigma)
             if w[0] ne -1 then plugmap[k[w]].objtype = 'REJECT5'
...
;           endif
         ...
         if keyword_set(skysubtract) and nother gt 3 and nccd gt 3 $
           ...
           if keyword_set(sky2d) then begin
             ...
             if (nccda le 3) then begin            
              ...
              if keyword_set(chelle) then low_sky=indgen((k[5]>59)+1) else low_sky=indgen((k[5]>74)+1) ;make sure we have the  nearest 6 skies, at least
              delvarx, xx
              traceset2xy, airset, xx,lam
              ...
              if (ccdnum eq 2) and (skysubq1 eq 1) then begin
                 if keyword_set(chelle) then sxaddpar, objhdr, 'SKYSUBQ1', 'Sky fit reverted to 1D for fibers 1-60' else sxaddpar, objhdr, 'SKYSUBQ1', 'Sky fit reverted to 1D for fibers 1-75'
              endif
              if ccdnum eq 2 then if keyword_set(chelle) then sxaddpar, objhdr, 'SKYSUBQ3', 'Sky fit reverted to 1D for fibers 121-180' else sxaddpar, objhdr, 'SKYSUBQ3', 'Sky fit reverted to 1D for fibers 151-225'
             endif else if (nccdb le 3) then begin

              k = where(strmatch(plugmap.objtype, 'SKY*'), kcnt)
              if keyword_set(chelle) then begin
                 hi_sky=indgen(120)
                 hi_sky=hi_sky[(k[(kcnt-6)]<60):*]
              endif else begin
                 hi_sky=indgen(150)
                 hi_sky=hi_sky[(k[(kcnt-6)]<75):*] ;make sure we have the  nearest 6 skies, at lea              
              endelse
              ...
              if (ccdnum eq 2) and (skysubq2 eq 1) then if keyword_set(chelle) then sxaddpar, objhdr, 'SKYSUBQ2', 'Sky fit reverted to 1D for fibers 61-120' else sxaddpar, objhdr, 'SKYSUBQ2', 'Sky fit reverted to 1D for fibers 76-150'
              if ccdnum eq 2 then if keyword_set(chelle) then sxaddpar, objhdr, 'SKYSUBQ4', 'Sky fit reverted to 1D for fibers 181-240' else sxaddpar, objhdr, 'SKYSUBQ4', 'Sky fit reverted to 1D for fibers 226-300'
           ...
           endelse
            ...
            pixmask = (objsubivar1 le 0.0d) ;newmask1  edited SM 09/17          
            objsub = objsub1
       ...
       if ccdnum eq 1 then begin
          ...
          nf2=n_elements(flux2[0,*])
          nf1=n_elements(flux1[0,*])
          nf=nf1+nf2
          specflux = dblarr(ncol, nf)
          specsky = dblarr(ncol, nf)
          specfluxivar = dblarr(ncol, nf)
          specflux(*,0:(nf1-1)) = flux1
          specflux(*,nf1:(nf-1)) = flux2
          if keyword_set(skysubtract) then begin
             specsky(*,0:(nf1-1)) = sky1
             specsky(*,nf1:(nf-1)) = sky2
          endif
          specfluxivar(*,0:(nf1-1)) = fluxivar1
          specfluxivar(*,nf1:(nf-1)) = fluxivar2
          
          pixmask = dblarr(ncol,nf)
          pixmask(*,0:(nf1-1)) = pixmask1
          pixmask(*,nf1:(nf-1)) = pixmask2
          
          coord1 = airset1.coeff
          coord2 = airset2.coeff
          coord = dblarr(n_elements(coord1[*,0]), nf)
          coord[*,0:(nf1-1)] = coord1
          coord[*,nf1:(nf-1)] = coord2
...
;             hs_scale, lam, specflux, sigma=3.0, scale=scale
        ...
        ;; Call standard proc to determine time-stamps       
          ...
          if ( size(ra, /tname) NE 'INT' $
             ...
             if keyword_set(chelle) then sxaddpar, objhdr, 'HELIO_RV', helio, $
              ' Heliocentric correction (km/s, not applied)' else $
       ...
       if j eq 0 and ccdnum eq 1 then begin
          ...
          if keyword_set(chelle) then nfib=240 else nfib=300
          npix=n_elements(lam[*,0])
          nfil=n_elements(filelist)
          lamout = dblarr(nfil, npix, nfib)
          fluxout = dblarr(nfil, npix, nfib)
          skyout = dblarr(nfil, npix, nfib)
          ivarout=  dblarr(nfil, npix, nfib)
          pixmaskout=  lonarr(nfil, npix, nfib)

          exptimes=fltarr(nfil)
;          headout = strarr(n_elements(filelist),$
;                           n_elements(objhdr))
          fullfitout=  dblarr(nfil, npix, nfib)
       endif
    endfor
    lamout[j,*, *] = lam
    fluxout[j,*, *] = specflux
...
endelse
       ...
       mwrfits, transpose(transpose(pixmaskout[j,*,*])), outfile
    ...
    if keyword_set(qaplot) and not keyword_set(chelle) then begin
 ...
 if keyword_set(fluxcorr) then begin
    ...
    if keyword_set(combine) then begin
       ...
       newmask = long(transpose(transpose(ivarout[0,*,*])) * 0.0)
       ormask = newmask
 ...
 if not keyword_set(fluxcorr) and keyword_set(combine) then begin   
    if keyword_set(noftweak) then begin
      ...
      if nim eq 1 then begin
       ...
       sxaddpar, objhdr, 'COMMENT', 'using HSRED v2.1 release 09/2017'
      endif else begin
       newflux=dblarr(npix,nfib)
       newfluxivar=newflux
       newflux_alt=newflux
       ...
       newmask=lonarr(npix,nfib) ;newflux
       ormask=newmask
       tempflux = fluxout 
       tempsky=skyout
       tempivar = ivarout 
       for ifib=0, nfib-1 do begin                
         ...
         for iexp=0, nim-1 do begin                
            if ((nim eq 3) and (iexp eq 1)) or ((nim eq 5) and (iexp eq 2)) or ((nim ne 3) and (nim ne 5) and (iexp eq 0)) then begin
              ...
              ;wavelength grid
              goodpix=where(ivarout[iexp,*,ifib] ne 0.0d, ngood) 
              ;if (ngood gt 1) and (max((fluxout[iexp,*,ifib])[goodpix])ge 2.d5) then $
              ;fudge=max((fluxout[iexp,*,ifib])[goodpix])/2.d5 else fudge=1.d
              ;hs_combine1fiber,transpose(lamout[iexp,*,ifib]), transpose(fluxout[iexp,*,ifib]/fudge), transpose(ivarout[iexp,*,ifib]), skyflux=transpose(skyout[iexp,*,ifib]), $
               ; newlam=transpose(templam), newflux=newflux1,$
               ; newivar=newivar1, newsky=newsky1, finalmask = transpose(pixmaskout[iexp,*,ifib]), $
               ; andmask = newmask1,nord=4

              ;switch to pp_resample, which conserves flux and does linear
              ;  interpolation across bad pixels
              newflux1 = pp_resample(transpose(fluxout[iexp,*,ifib]*ivarout[iexp,*,ifib]),transpose(lamout[iexp,*,ifib]),transpose(newlam[*,ifib]))
              newivar1 = pp_resample(transpose(ivarout[iexp,*,ifib]),transpose(lamout[iexp,*,ifib]),transpose(newlam[*,ifib]))
              newflux1=newflux1/newivar1
              newivar1[where(newivar1 le 0.0d,/null)]=0.0d
              newflux1[where(newivar1 eq 0.0d,/null)]=0.0d
              newmask1 = pp_resample(transpose(float(pixmaskout[iexp,*,ifib])),transpose(lamout[iexp,*,ifib]),transpose(newlam[*,ifib]))
              newmask1[where(newmask1 gt 0, /null)]=1
              newmask1=long(newmask1)
              newflux1[where(newmask1,/null)]=0.0
              newivar1[where(newmask1,/null)]=0.0
              newmask1[where(newivar1 eq 0.0,/null)]=1
              newflux1 = djs_maskinterp(newflux1, newmask1, /const)
              ;goodpix2=where(newivar1 gt 0.0d, /null)
;print, 'S/N orig: ',total(fluxout[iexp,goodpix,ifib]*sqrt(ivarout[iexp,goodpix,ifib])), 'S/N resampled: ', total(newflux1[goodpix2]*sqrt(newivar1[goodpix2]))
              ;stop
              newsky1 = pp_resample(transpose(skyout[iexp,*,ifib]),transpose(lamout[iexp,*,ifib]),transpose(newlam[*,ifib]))
              newsky1[where(newmask1,/null)]=0.0
              newsky1 = djs_maskinterp(newsky1, newmask1, /const)
            ...
            if iexp eq 0 then ormask[*,ifib]=newmask1 else ormask[*,ifib]=(newmask[*,ifib] + newmask1)
         endfor
         newflux[*,ifib]=total(tempflux[*,*,ifib], 1) ;*float(nim)/float(nim-ormask[*,ifib])
         newsky[*,ifib]=total(tempsky[*,*,ifib], 1) ;*float(nim)/float(nim-ormask[*,ifib])
...
;stop
 ...
 if keyword_set(writeout) then begin
    ...
    sxaddpar, objhdr, 'EXTEND', 'T', after='NAXIS2'
    mwrfits, lamout, outname, /create, objhdr
    mwrfits, fluxout, outname
    mwrfits, ivarout, outname
    mwrfits, pixmaskout, outname
    
    
    if keyword_set(combine) then mwrfits, ormaskout, outname
    if NOT keyword_set(combine) then mwrfits, pixmaskout, outname
