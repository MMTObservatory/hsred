...
;        fwhm[j]=A[2]
    ...
    fwhmmask=inmask
    used=where(inmask,used_cnt)
    if used_cnt gt 1 then $
      ratios[i]=median(fwhm[used]/fwqm[used]) else $
      if used_cnt eq 1 then ratios[i]=fwhm[used]/fwqm[used] else ratios[i]=1.
