...
pro hs_combine1fiber, inloglam, objflux, objivar, $
 ...
 _EXTRA=KeywordsForReject, verbose=verbose
   ...
   newflux = dblarr(nfinalpix)
   newmask = lonarr(nfinalpix)
   if (arg_present(newivar)) then newivar = dblarr(nfinalpix)
   if (arg_present(newdisp)) then newdisp = dblarr(nfinalpix)
   if (arg_present(newsky)) then newsky = dblarr(nfinalpix)
   if (arg_present(newdisp) OR arg_present(newsky)) then $
    newdispweight = dblarr(nfinalpix)
         ...
         if (n_elements(ss) GT 2) then begin
            if (keyword_set(objivar)) then $
             sset = bspline_iterfit(inloglam[ss], objflux[ss], $
              nord=nord, /groupbadpix, bkspace=bkptbin, $ 
              bkpt=bkpt, invvar=objivar[ss], outmask=bmask, $
              _EXTRA=KeywordsForReject, /silent) $
            else $
             sset = bspline_iterfit(inloglam[ss], objflux[ss], $
              nord=nord, /groupbadpix, bkspace=bkptbin,$
              bkpt=bkpt, outmask=bmask, $
              _EXTRA=KeywordsForReject, /silent)
         endif else begin
...
; Another work-around for the Slatec code ???
         if (numinside LE 2) then begin
            ...
            newflux[inside] = bspline_valu(newlam[inside], sset, $
                                   mask=bvalumask)
            goodvalu = where(bvalumask,/null)
            if goodvalu NE !null then newmask[inside[goodvalu]] = 1
            ...
            ireplace = where(bmask EQ 0,/null)

            if (ireplace NE !null) then begin 
      ...
      for j=0L, max(specnum) do begin
         these = where(specnum EQ j,/null)

         if (these NE !null) then begin

            inbetween = where(newlam GE min(inloglam[these]) AND $
                              newlam LE max(inloglam[these]),/null)
            if (inbetween NE !null) then begin
               ...
               if (arg_present(newivar)) then begin
                  ...
                  smask = interpol(double(fullcombmask[these]), $
                   inloglam[these], newlam[inbetween])
                  ibad = where(smask LT 1.0 - EPS, /null)
                  if (ibad NE !null) then result[ibad] = 0
   ...
   if (keyword_set(newivar)) then begin
      badregion = where(smooth(newivar,3) EQ 0, /null) 
      if badregion NE !null then begin
   ...
   if (keyword_set(newivar)) then $
    inff = where(finite(newflux) EQ 0 OR finite(newivar) EQ 0, /null) $
   else $
    inff = where(finite(newflux) EQ 0, /null)

   if (inff NE !null) then begin
   ...
   if (keyword_set(newivar)) then begin
      ...
      ; Interpolate over masked pixels, just for aesthetic purposes
       ...
       goodpts = where(newivar GT 0, /null)
       if goodpts NE !null then begin
         minglam = min(newlam[goodpts])
         maxglam = max(newlam[goodpts])
         
         ibad = where(newlam LT minglam OR newlam GT maxglam, /null)
         if (ibad NE !null) then begin
