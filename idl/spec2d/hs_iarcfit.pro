...
pro hs_iarcfit, arc, arcinvvar, wset, xcen, ycen, datapath=datapath, $
  ...
  if keyword_set(chelle) then norder_arc = 5L
  ...
  crit = 100
  IF keyword_set(chelle) THEN crit = 200
...
;  qtrim = (xstart gt -5L) and (xstart lt ncols+4L) and lamp.good
...
; initial line centers
  ...
  if keyword_set(debug) then begin
     ...
     djs_oplot, findgen(ncols), arc[*,0], color='yellow'
     djs_oplot, findgen(ncols), arc[*,nrows-1], color='red', ps=10
     ...
     if keyword_set(doplot) or keyword_set(write) then begin
        ...
        for i = 0L, iter do begin
           ...
           lineid_plot, xwave, yflux, lamp[xx].lambda, lamp[xx].element, $
              ...
              xtitle='Wavelength ('+angstrom()+')'+'Nlamp:'+string(nlamp), $
