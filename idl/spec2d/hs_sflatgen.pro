...
pro hs_sflatgen1, files, outfile=outfile, outdir=outdir, $
  ...
  for iamp = 1, 4 do begin
     ...
     if nfile gt 1 then $
       mnimg = djs_avsigclip(imgarr, sigrej=sigrej, maxiter=maxiter, inmask=inmask) else $
          mnimg=imgarr
...
PRO hs_sflatgen, sflatfiles,  outdir=outdir, indir=indir, gzip=gzip, mjd=mjd, chelle=chelle
  ...
  for ifile = 0, n_elements(sflatfiles) - 1l do begin
     ...
     if not keyword_set(chelle) and (imtype ne 'skyflat') then begin
