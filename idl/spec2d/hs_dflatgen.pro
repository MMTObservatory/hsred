...
pro hs_dflatgen1, files, outfile=outfile, outdir=outdir, $
  ...
  for ifile =0, nfile-1 do begin
     ...
     if nfile gt 1 then mnimg = djs_avsigclip(imgarr, sigrej=sigrej, maxiter=maxiter, $
                           inmask=inmask) else mnimg=imgarr
...
PRO hs_dflatgen, dflatfiles,  outdir=outdir, indir=indir, gzip=gzip, mjd=mjd, apflat=apflat
  ...
  if stop ne 'stop' then begin
     if keyword_set(apflat) then dflatname='apflat.fits' else dflatname = 'dflat.fits'
