...
if keyword_set(multispec) then begin
  ...
  if not keyword_set(dx) then dx=1.2 ;dx=(wavemax-wavemin)/n_elements(lam[*,0])
  
  npix = long( (wavemax-wavemin)/dx) + 1
  newlam = alog10(wavemin + dx * lindgen(npix))
  newflux = dblarr(npix, nfib)
  newivar = dblarr(npix, nfib)
  newsky=dblarr(npix, nfib)
  newflux_box=dblarr(npix, nfib)
     ...
     if keyword_set(multispec) then begin
        ...
        if ngood gt 1 then begin
         ;if max(flux[goodpix,i]) ge 2.d5 then fudge=max(flux[goodpix,i])/2.d5 else fudge=1.d
          ;stop
          ;fudge=1.0d
          ;hs_combine1fiber, alog10(lam[goodpix,i]), flux[goodpix,i]/fudge, ivar[goodpix,i], skyflux=sky[goodpix,i], $
          ;newlam=newlam, newflux=newflux1, newivar=newivar1, newsky=newsky1, nord=3,/verbose, upper=10, lower=10,maxiter=0, maxrej=0
          ;hs_combine1fiber, alog10(lam[goodpix,i]), flux_box[goodpix,i]/fudge, ivar[goodpix,i], newlam=newlam,newflux=newflux2, nord=3,maxiter=0,maxrej=0,/verbose, upper=10, lower=10
          ;newsky[*,i]=newsky1
          ;newflux_box[*,i]=newflux2*fudge
          ;newflux[*,i] = newflux1*fudge
          ;newflux_box[*,i]=interpol(flux_box[goodpix,i],alog10(lam[goodpix,i]),newlam,/spline)
          ;newflux[*,i] = interpol(flux[goodpix,i],alog10(lam[goodpix,i]),newlam,/spline)
         ...
         if ngood gt 1 then begin
          ...
          newivar[*,i] = pp_resample(ivar[goodpix,i],alog10(lam[goodpix,i]),newlam)
           ...
           ; newlam=newlam, newflux=newflux1, newivar=newivar1, nord=4, maxiter=0
           ;newflux[*,i] = newflux1*fudge
           ;newivar[*,i] = newivar1
        endif
      endelse
  ...
  if keyword_set(multispec) then   begin
     sxaddpar, header, 'NAXIS3', 4, after='NAXIS2'
     sxaddpar, header, 'EXTEND', 'T', after='NAXIS3'
  endif else begin
...
if NOT keyword_set (multispec) then begin
  ...
  if keyword_set(hsplit) then begin
    ...
    for i=0, nfib-1 do begin   
       ...
       id=' '+strn(fix(plugmap[i].icode))+' '
       if strtrim(strlowcase(plugmap[i].objtype),2) eq 'sky' then id=' -1 ' else if (strtrim(strlowcase(plugmap[i].objtype),2) ne 'target') and strtrim(strlowcase(plugmap[i].objtype),2) ne 'opptar' then id=' 0 '  
       apidstring=strtrim(plugmap[i].bcode,2)+' '+hs_dec2hms(plugmap[i].ra/15., /double)+' '+hs_dec2hms(plugmap[i].dec, /double)+id+string(fix(plugmap[i].rcode), format='(I3)') +' '+string(plugmap[i].xfocal, format='(f8.3)')+' '+string(plugmap[i].yfocal, format='(f8.3)')
       sxaddpar, hdr1d, 'APID'+strn(1), apidstring 
;       if strtrim(strlowcase(plugmap[i].objtype),2) eq 'target' then begin
       if (plugmap[i].icode gt 0) or (strtrim(strlowcase(plugmap[i].objtype),2) eq 'opptar') then begin
