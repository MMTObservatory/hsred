...
PRO hs_preproc, inpath=inpath, overwrite=overwrite, plugdir=plugdir
  ...
  if file_test('lists/dflat.list') NE 0 AND $
     ...
     k = where(strmatch(strupcase(imagetyp), 'DOMEFLAT*') and strmatch(strupcase(object), 'RING*'), ct)
     if ct eq 0 then begin
        k = where(strmatch(strupcase(imagetyp), 'DOMEFLAT*') or strmatch(strupcase(object), 'DOMEFLAT*') or strmatch(strupcase(files), '*DOMEFLAT*'), ct)
        if ct gt 0 then begin 
  ...
  ;;Get the domeflats with object config and make apflat.list
  
  if file_test('lists/apflat.list') NE 0 AND $
     NOT keyword_set(overwrite) then begin
     
     splog, 'Apflat File lists/apflat.list already exists.  Will not '
     splog, 'overwrite'
  endif else if (file_test('lists/apflat.list') NE 0 AND $
                 NOT keyword_set(overwrite)) OR $
     file_test('lists/apflat.fits') $
     EQ 0 Then begin
     
     k = where(strmatch(strupcase(imagetyp), 'DOMEFLAT*') and not strmatch(strupcase(object), 'RING*'), ct)
     if ct eq 0 then begin
           splog, 'No images with IMAGETYP=DOMEFLAT and OBJECT!=Ring config FOUND. '
     endif else begin
        
        openw, 1, 'lists/apflat.list'
        printf, 1, '#FILE      OBJECT     IMAGETYPE'
        FOR I = 0, CT-1 DO BEGIN
           printf, 1, files(k(i)) + '   ' + object(k(i)) $
              + '   ' +  imagetyp(k(i))
  ...
  if file_test(inpath + 'lists/cal.list') NE 0L and $
     ...
     for i = 0, nmaps -1 do begin
        ...
        for j =0, n_elements(files) -1 do begin
           ...
           filetrim(j) = strmid(files(j), incount, n-incount-10-3*(gz_str[k[j]] eq '.gz'))
        endfor
        ...
        if ct eq 0 then begin
          ...
          for j =0, n_elements(filetrim) -1 do begin
             ...
             filetrim[j] = strmid(files[k[j]], incount, n-incount-10-3*(gz_str[k[j]] eq '.gz'))
          endfor
          filetrim=filetrim[sort(filetrim)]
          filetrim=filetrim[uniq(filetrim)]
          for j =0, n_elements(filetrim) -1 do begin
             match = strmatch(files[k],inpath+filetrim[j]+'.????.fit*')
